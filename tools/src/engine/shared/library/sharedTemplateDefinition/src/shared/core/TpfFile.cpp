...
int TpfFile::WriteIffFile(Iff & iffData, const Filename & fileName)
{
	...
	fileName.verifyAndCreatePath();
		...
		if (WindowsUnicode)
		{
			...
			delete[] buffer;
			srcPath = Unicode::narrowToWide("\\\\?\\") + srcPath;  // Utinni 12-02: char16_t Unicode::String under C++20
			...
			delete[] buffer;
			destPath = Unicode::narrowToWide("\\\\?\\") + destPath;  // Utinni 12-02: char16_t Unicode::String under C++20
