...
{
#ifdef UTINNI_TOOLS_NO_PERFORCE
	// Utinni 13-01 (AUTH-06): no-op stub. p4 is build-farm VCS bookkeeping, not data;
	// the .tpf/.iff outputs are written by fopen in run() independent of Perforce.
	// Returning true marks the file "already checked out" so the caller's
	// addFileToPerforce() branch is skipped. See Directory.Build.props rationale.
	(void)filename;
	return true;
#else
	...
	return true;
#endif // UTINNI_TOOLS_NO_PERFORCE
}
...
{
#ifdef UTINNI_TOOLS_NO_PERFORCE
	// Utinni 13-01 (AUTH-06): no-op stub — see getFileFromPerforce above.
	(void)filename;
	return true;
#else
	std::string fixedname(filename);
