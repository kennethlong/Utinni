...
overwritten. The file is ~27,600 lines of:
...
don't edit by hand. (The banner carries **no** generator-version string, so
the ABI gate keys off the file's *structural public surface*, not a version
line — see "Validating a regen" below.)
...
1. **Rebuild `UtinniCoreDotNet`** — the easiest smoke test. Compile errors
   ...
   - Renamed types — hand-written managed code that calls into the bindings
     needs to update.
2. **Check the ABI surface, not a raw text diff.** CppSharp reorders the
   whole file on every build (a huge symmetric no-op diff — which is why the
   policy is to `git checkout -- Generated/UtinniCore.cs` and never commit
   it). A raw diff is therefore pure noise. Instead the ABI gate keys off the
   file's *structural public surface* — namespace/class FQNs, public
   method/property signatures, every `[DllImport(... EntryPoint="<mangled>")]`
   anchor, and enum members / field layout — normalized and hashed **per
   block** into an order-independent set, compared against a committed
   baseline. Reorder churn produces the identical hash set (PASS); an
   added/removed/re-signatured public member changes the set (FAIL).
...
### Re-blessing an intentional surface change

When a regen *intentionally* moves the public surface (a new function, a
signature change), the per-block-hash gate goes red on purpose. Re-bless it —
and rebuild the dependents in lockstep, because a moved binding surface
detonates pre-built plugin DLLs at MEF compose (a defaulted/`[Caller*]` param
is source-compatible but **not** binary-compatible):

1. Rebuild **The Jawa Toolbox** (and any sibling plugin in
   `D:/Code/UtinniPlugins`) against the new `UtinniCoreDotNet`.
2. Regenerate the committed ABI baseline block-hash file via the diff tool's
   `--rebless` mode.
3. Rebuild and re-freeze the frozen-plugin compose fixture.
4. Commit all three artifacts together in the same change.

> The `--rebless` tool and the frozen-plugin compose fixture are built in
> Plan 03 of Phase 17; this is the procedure they implement.

### The redirect is the supported config

The generator parses with vendored clang 11 against the **VS 2019 14.29 STL**
while the C++ build uses v145 — see
[cppsharp-v145-redirect.md](cppsharp-v145-redirect.md) for why that redirect is
load-bearing (no released CppSharp parses the v145 STL), its ABI-stability
assumption, the toolchain prerequisites, and the `UTINNI_VS2019_ROOT` /
`UTINNI_SLN_DIR` env overrides.
