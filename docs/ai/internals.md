...
| `0x01919410` *(data)*  | `Game::isSafeToUse` flag #2                    | direct read         | ...this. EITHER being set means safe. (2026-07-19 live: in a fully-loaded in-world session one flag stays unset; the earlier "both must be true" model made isSafeToUse false and silently blocked all world-snapshot mutations — the remove regression. OR is the field-proven semantics. Confirmation smoke measured `safeFlags={0,1}`: flag #1 `0x01908858` is the one that stays UNSET in-world on Core3; flag #2 `0x01919410` is set.) |
