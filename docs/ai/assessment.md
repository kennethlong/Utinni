...
8. **DXSDK June 2010 dependency** — could it be replaced with Windows 10
   ...
   **CON-O-08 — RESOLVED Phase 6 (Plan 06-03 Task 1):** DXSDK June 2010
   removed entirely. Utinni used no `d3dx9` math helpers beyond a single
   `D3DXVECTOR3` (the dummy point for the RESZ depth-resolve draw in
   `depth_texture.cpp`), now a local 3-float `Vec3` struct with identical
   byte layout. DXSDK include/lib paths stripped from `UtinniCore.vcxproj`
   across all three configs; the CI "Verify DirectX SDK (June 2010)" step
   removed. New GPU-side vector/matrix math should use DirectXMath
   (`<DirectXMath.h>`, Windows SDK). Structurally closes CON-B-03 (DXSDK
   paths in all configs).
