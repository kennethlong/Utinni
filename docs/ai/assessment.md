...
**Phase 02.1 closure summary:** All 8 findings (CR-02, CR-03, CR-04, WR-01, WR-02, WR-03, WR-05, WR-09) closed in code. Every fix has a regression test that would fail if the fix were reverted (D-04 satisfied). Additionally, a new utinni_test_resolveExports harness (`d13de0a`) was added to close the C-01 export-decoration gap that the LoaderLockHarness missed — see ExportResolutionTests.cs. WR-02 is marked partial-proof in Release/MT (see notes); full proof deferred to Phase 6 STAB-03.
