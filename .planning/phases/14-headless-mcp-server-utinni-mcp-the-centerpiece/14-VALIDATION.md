...
| 14-03a-01 | 14-03a | 1 | MCP-02 (SC2) | T-14-15 | `apply-save-{tab,iff,stf,ot}` applies ONE typed edit → verifies untouched-byte identity → `WriteAtomic`-commits the MUTATED bytes; **read-back asserts the edited value is present + file hash changed** (a no-op re-serialize MUST fail); failed verify = exit 2, no write | unit (golden) | `dotnet test Utinni.Cli.Tests/Utinni.Cli.Tests.csproj --filter ApplySave` | ❌ W0 | ⬜ pending |
| 14-03-01 | 14-03 | 3 | MCP-02 (SC2) | T-14-02 | `save_*` wrap the single `apply-save-*` verb opaquely; persist-vs-fail decided on CLI **exit code only** (host never parses `bytesEqualUntouched`); save returns `{written,path,bytesWritten,validated}` | unit | `dotnet test Utinni.Mcp.Tests/Utinni.Mcp.Tests.csproj --filter SaveCompositionTests` | ❌ W0 | ⬜ pending |
...
| 14-04-01 | 14-04 | 4 | MCP-02 (SC5) | — | edit→save writes loose-override file, **then reads the asset back and asserts the edited value is present + file hash changed before/after** (`RoundTripTests.EditSaveRoundTrip`); a no-op re-serialize MUST fail this test | integration | `dotnet test Utinni.Mcp.Tests/Utinni.Mcp.Tests.csproj --filter RoundTripTests` | ❌ W0 | ⬜ pending |
