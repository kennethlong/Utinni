...
|------------|------------|-----------|---------|----------|
| `D:\Sample-TRE-Files` (COT2000 + 46× v6000 `.tre`, 213k paths) | v6000/COT2000 enumeration golden tests | ✓ | SwgRestoration set | — |
| Synthesized 0005/0006 TRE fixtures (in-repo) | existing CLI goldens | ✓ | `Utinni.Cli.Tests/Fixtures/tre/` | — |
| Synthesized + loose IFF fixtures (in-repo) | IFF chunk-tree goldens | ✓ | `Utinni.Cli.Tests/Fixtures/iff/`, `world-snapshot/` | — |
| Real SWGEmu 0004/0005/0006 `.tre` | confirming Open Q1 record order | ✗ | — | Keep existing size-first path for 0004/0005/0006 until a fixture is sourced from the user's SWGEmu install |
| `swg-main/serverdata` loose IFF | per-type decoder goldens (datatable/STF/template/mesh) | ? (not probed) | — | Extract readable IFFs from an SWGEmu client via the new reader; author small in-repo fixtures |
| `swg-client-v2` reference repo | format-spec port (D-02) | ✓ | `D:\Code\swg-client-v2` | — |
| `TreeFileExtractor.exe` | v6000 payload extraction (deferred per D-07) | ✗ | — | enumerate-only banner (D-07); not needed for V1 read scope |
| `python` (for fixture-authoring scripts only) | optional fixture tooling | ✓ | C:\Python314 / C:\Python312 | not a runtime dep (D-02) |

**Missing dependencies with no fallback:** none block the phase. v6000 payload decode is intentionally out of scope (D-07).
**Missing dependencies with fallback:** real SWGEmu `.tre` (confirm Open Q1) and `swg-main/serverdata` (decoder fixtures) — both have viable fallbacks above.

## Validation Architecture

> Nyquist validation is enabled (no `workflow.nyquist_validation:false` in config). Phase 7's read paths are highly testable — the explicit design goal of TEST-03 (CLI + golden fixtures) is to make exactly this kind of work unattended-verifiable.

### Test Framework

| Property | Value |
|----------|-------|
| Framework | xUnit (`UtinniCoreDotNet.Tests`, Tier-1 parser tests) + golden-file tests (`Utinni.Cli.Tests`, Tier-2) — both .NET Framework 4.7.2, run via `dotnet test` |
| Config file | existing `.github/workflows/ci.yml` runs two `dotnet test` lanes per push (Phase 1 + Phase 4) |
| Quick run command | `dotnet test Utinni.Cli.Tests` (or the parser-test project) |
| Full suite command | `dotnet test Utinni.sln` (both lanes) + the Catch2 native exe (Phase 5) |

### Phase Requirements → Test Map

| Req ID | Behavior | Test Type | Automated Command | File Exists? |
|--------|----------|-----------|-------------------|-------------|
| PROD-W1-TRE | enumerate v0005/v0006 TRE (existing path stays green) | golden | `dotnet test Utinni.Cli.Tests --filter parse-tre` | ✅ |
| PROD-W1-TRE | enumerate v6000 per-`.tre` (32-byte TOC, zlib) | golden | new test vs `D:\Sample-TRE-Files\SwgRestoration_06.tre` | ❌ Wave 0 |
| PROD-W1-TRE | enumerate COT2000 master index (213k paths, 45 archives) | golden/smoke | new test vs `SwgRestoration.toc` (assert count=213086, 45 tre names) | ❌ Wave 0 |
| PROD-W1-TRE | zlib-framed block inflate vs raw-deflate fixture | unit | new test: feed `0x78 0x9c`-framed block + raw-deflate block, both inflate | ❌ Wave 0 |
| PROD-W1-TRE | 5000 tag recognized, degrades to enumerate-only (no crash) | unit (gated) | new test: synthesized 5000-header fixture → enumerate-only result | ❌ Wave 0 |
| PROD-W1-TRE | lazy enumeration does not eager-read payloads | unit | new test: enumerate without reading payload; assert no full-archive read | ❌ Wave 0 |
| PROD-01 | universal IFF chunk tree over any IFF | golden | `dotnet test Utinni.Cli.Tests --filter inspect-iff` (existing) + new real-IFF fixture | ✅ / ❌ Wave 0 |
| PROD-01 | datatable structured decode (DTII→COLS/TYPE/ROWS) | golden | new decoder test vs a small `.iff` datatable fixture | ❌ Wave 0 |
| PROD-01 | STF / object-template / mesh-summary decode | golden | new decoder tests vs small fixtures (from swg-main or extracted) | ❌ Wave 0 |
| PROD-W1-TRE #4 | CLI + browser share the code path | structural | assert browser calls only `Formats/` APIs that CLI verbs also call (review/grep gate) | ❌ Wave 0 |
| PROD-W1-TRE #1 | subpanel loads inside TJT against live SWG | manual (Tier 4) | documented residual — live-SWG smoke (human checkpoint per write-authority memory) | manual |

### Sampling Rate
- **Per task commit:** `dotnet test` on the touched test project (parser or CLI goldens) — < 30 s.
- **Per wave merge:** full `dotnet test Utinni.sln` (both lanes).
- **Phase gate:** both `dotnet test` lanes green + the live-SWG TJT panel smoke (Tier-4 manual residual, TEST-04) before `/gsd:verify-work`.

### Wave 0 Gaps
- [ ] `Utinni.Cli.Tests/Fixtures/tre/` — add a small v6000 fixture (or test directly against `D:\Sample-TRE-Files` behind an env-gated trait) covering the 32-byte/zlib path
- [ ] `Utinni.Cli.Tests` (or parser tests) — zlib-vs-raw-deflate inflate test
- [ ] synthesized 5000-header fixture + gated enumerate-only test
- [ ] lazy-enumeration assertion (no eager payload read)
- [ ] COT2000 master-index golden (count + tre-name assertions) — large fixture, gate on `D:\Sample-TRE-Files` presence (env var `SWG_SAMPLE_TRE_DIR`)
- [ ] per-type decoder fixtures: small datatable / STF / object-template `.iff` (acquire from swg-main or extract via the new reader)
- [ ] real SWGEmu 0004/0005/0006 fixture to confirm Open Q1 (record field order) — acquisition task
- [ ] CLI verb coverage for any new decode path so CLI+browser stay in lock-step (criterion #4)

*(Existing infra covers v0005/v0006 metadata + IFF chunk-tree goldens already.)*

## Security Domain

> `security_enforcement` is not set to false in config (absent ⇒ enabled). Phase 7 is a local desktop file reader of attacker-influenceable binary input (`.tre`/`.iff` files a user may download). The relevant threat surface is **untrusted-input parsing**, not network/auth/crypto.
...
| V4 Access Control | no | Local file access only; no privilege boundary. |
| V5 Input Validation | **yes** | Existing TRE/IFF readers already enforce: non-negative length checks, 256 MB (TRE) / 64 MB (IFF) caps (T-04-DoS), bounds checks on every offset, deflate-expansion caps. EXTEND these to the new v6000/COT2000 fields (treeFileIndex bounds, fileNameOffset bounds, TOC-size-vs-stream-length, name-block-size consistency). |
| V6 Cryptography | no (read) | v6000 payloads are encrypted but Phase 7 does NOT decrypt (D-07 enumerate-only). No crypto implemented. Never hand-roll a deobfuscator. |

### Known Threat Patterns for SWG binary-file parsing

| Pattern | STRIDE | Standard Mitigation |
|---------|--------|---------------------|
| Malicious TRE with huge declared sizes (decompression bomb / OOM) | DoS | Existing `MaxBlockSize` (256 MB) cap + deflate-expansion-exceeds-cap check; apply same caps to COT2000 TOC/name blocks and per-`.tre` blocks. |
| Out-of-bounds offsets (infoOffset, nameOffset, treeFileIndex, fileNameOffset) | Tampering / DoS | Bounds-check every offset against stream length and block length before use (existing `ValidateLength` pattern); validate `treeFileIndex < numTreeFiles`. |
| Truncated / malformed archive | DoS | Short-read detection (existing `Truncated` error kind); structural validation before decode. |
| Path-traversal via crafted virtual paths (e.g. `../`) when displaying/extracting | Tampering | Phase 7 is read-only browse (no extraction in V1) — display paths as data, never resolve them against the filesystem. If a "save payload to disk" affordance is added, sanitize. |
| Untrusted zlib stream | DoS | Cap inflate output (existing pattern); zlib-header-strip must validate the header (`%31==0`) before trusting it. |

## Sources

### Primary (HIGH confidence)
- **Utinni codebase (verified by reading + byte-level decode):**
  - `UtinniCoreDotNet/Formats/Tre/{TreFile,TreHeader,TreRecord,TreParseException}.cs` — eager-read model, size-first record order, `DeflateStream` raw-deflate path, 0005/0006-only version gate.
  - `UtinniCoreDotNet/Formats/Iff/{IffReader,IffDocument,IffChunk,IffLeafChunk,IffContainerChunk}.cs` — EA-IFF-85 reader, BE tags, leaf `.Data`, container `.Children`, parser-purity rule.
  - `Utinni.Cli/{Program.cs,Commands/ParseTreCommand.cs,Commands/ListObjectsCommand.cs}` — verb dispatch, shared-core consumption, sorted-key JSON, `OBJS` byte-scan debt.
  - `UtinniCore/swg/misc/{repository.h,repository.cpp,tree_file.h,tree_file.cpp}` + `UtinniCoreDotNet/Generated/UtinniCore.cs` `Repository` binding — live one-shot destructive harvest, `Game.getRepository()` singleton, `GetDirectoryInfo`/`GetFilenameAt`/`FilenameCount`.
  - `UtinniCoreDotNet/PluginFramework/IEditorPlugin.cs`, `UI/Controls/SubPanel.cs` — MEF SPI, 417px SubPanel base.
  - `Utinni.Cli.Tests/Fixtures/{tre,iff,world-snapshot}/` — synthesized fixtures; decoded `synthesized-3record-v0005.tre` confirming size-first order + raw-deflate framing.
- **UtinniPlugins codebase:** `TheJawaToolboxDotNet/UI/Forms/FormObjectBrowser.cs` (themed TreeView + async LoadRepo + AfterSelect), `Plugin.cs` (SubPanelContainer registration).
- **swg-client-v2 reference (read in full, D-02):** `tools/swg_blender/swg_pipeline/tre_reader.py` (TRE/COT2000/SearchTOC layouts), `tools/swg_blender/swg_iff/reader.py` (IFF navigation, BE tags / LE scalars), `docs/research/sample-tre-files.md` (COT2000 + v6000 byte layout), `docs/research/iff-tre-codebase-map.md` (engine loader index), `tools/swg_blender/PHASE6_VERIFICATION.md` (5000/0006 unsupported-until-fixture), `src/engine/shared/library/sharedUtility/src/shared/DataTable.cpp` (datatable load_0000/0001).
- **Live fixture verification:** `D:\Sample-TRE-Files\SwgRestoration_06.tre` decoded directly (v6000, 32-byte crc-first TOC, zlib `0x78 0x9c` framing, 54 valid file paths); 46 archives confirmed all-v6000 (no 5000).

### Secondary (MEDIUM confidence)
- `.planning/REQUIREMENTS.md` (PROD-W1-TRE, PROD-01), `.planning/ROADMAP.md` §Phase 7, `.planning/PROJECT.md` (DEC-C4, DEC-A3), `.planning/intel/constraints.md` (CON-N-02, CON-M-01/02, CON-T-05) — phase scope + preservation.
- MEMORY.md project notes: TRE version support gap, swg-client-v2 reference corpus, HUD-style overlay directive (per-panel revisitable), GSD worktrees off, grep-gate hygiene, cross-repo write authority.

### Tertiary (LOW confidence)
- WebSearch: `.NET DeflateStream` zlib-header workaround (dotnet/runtime #2236, #16923, #94246) — corroborates the verified byte-level finding that `.tre` blocks are zlib-framed and `DeflateStream` needs the header stripped.

## Metadata

**Confidence breakdown:**
- Standard stack: HIGH — all in-box/in-repo; no external deps; versions verified in code.
- Architecture: HIGH — patterns read verbatim from existing code; data flow verified end-to-end.
- TRE format / version dispatch: HIGH for 0005/0006 (existing fixtures) and 6000/COT2000 (decoded a real archive); MEDIUM for the 0004/0005/0006 *engine* order (Open Q1 — needs a real SWGEmu fixture); LOW for 5000 (no fixture exists anywhere — defensive only).
- Per-type decoders: MEDIUM-HIGH — datatable layout verified against engine source; STF/object-template/mesh layouts have authoritative references (D-09) but not byte-verified this session.
- Pitfalls: HIGH — field-order divergence and zlib-vs-raw-deflate both proven by direct decode.

**Research date:** 2026-05-26
**Valid until:** 2026-06-25 (stable domain — game file formats don't change; the only volatile item is whether a 5000 fixture surfaces, which would *upgrade* confidence).
