...
last_activity: 2026-05-19 night -- EB FE jmp-self halt RESOLVED; SWG boots through to login screen
progress:
  total_phases: 12
  completed_phases: 3
  total_plans: 9
...
9. **NEW: Cursor doesn't display + special keys (delete/tab/return) don't work in game** — First successful boot tonight (2026-05-19 ~20:09): regular character keys typed into the login screen username field worked, but special keys (delete, tab, return) and mouse cursor visibility did not. `hkSetupInstall` log line confirms: `(passthrough HWND; cursor side-effects retained)`. **Open questions:** does DirectInput have focus on SWG's HWND or the editor's? Are keys flowing through `WM_CHAR` only (which would bypass DirectInput's keyboard device)? Is SWG hiding the system cursor and trying to show its own custom cursor that's never actually rendered? **Next steps:** (a) VS attach + examine DirectInput device acquisition state in SWG; (b) hook `ShowCursor` to log calls and return values; (c) compare `WM_KEYDOWN` routing between standalone SWG and Utinni-injected sessions.
