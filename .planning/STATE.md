...
> - **Consumer work GATED on provider v3 (the single post-v3 pass):** re-sync the v3 .h/.inc, add the 16 new bindings + bump the endpoints_bindings.cpp static_asserts (78→94, 77→…), drop the matching `installable()`/group skips, wire the 3 object-virtual LIVE self-checks + the advertised-client lazy-hook (instance-timing), then maintainer live-smoke. The v2 94-name .h/.inc re-sync is intentionally NOT committed to Utinni yet (it trips the 78-name guard); re-apply at the start of the post-v3 pass.
> - **Repo ownership:** the Utinni instance owns all `D:/Code/Utinni` consumer edits; the provider instance stands down on the Utinni repo (maintainer-confirmed 2026-06-22, after its re-sync twice collided with the consumer build).
