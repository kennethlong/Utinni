...
last_activity: 2026-05-21 morning -- Issue #10 Phase B-bis LANDED. SWG now resizes with PanelGame end-to-end (drop SWP_NOSIZE, drop SWP_NOZORDER, use HWND_TOP in SetWindowPos). Verified live: SWG fills PanelGame at all sizes, drag tracks, login + Naboo scene load + WASD movement + chat enter + Esc untarget + clean /quit. Handoff sketch (pDevice->Reset) was unworkable -- SWG holds untracked default-pool resources, Reset returns D3DERR_INVALIDCALL and crashes the device. Actual mechanism: D3D9 windowed COPY swapchain handles backbuffer-vs-window mismatch via its own stretching; the only real fix needed was Z-order (owned-popup post-creation GWLP_HWNDPARENT doesn't trigger Z recompute, SWG got buried behind FormMain).
progress:
  total_phases: 12
  completed_phases: 3
  total_plans: 9
