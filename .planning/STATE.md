...
6. **NEW (2026-05-19 late afternoon): SWG main thread halts in jmp-self at `0x0131DC7A`** — VS attach to running SWGEmu (post-preloading) revealed main thread (TID 60040, marked Main Thread) has `EIP=0x0131DC7B` parked, with `ECX/EDX/ESI/EDI` all holding `0x0131DC7A` — the function-entry address. Multiple Break-All cycles show EIP unchanged → thread genuinely in infinite loop. **Memory bytes at `0x0131DC7A` differ from file**: file has `55 8B EC ...` (push ebp; mov ebp, esp; ... — normal MSVC prologue with SEH at `+3` and indirect call to IAT slot `[0x015DC0E0]` at `+0x26`); memory has `EB FE EC ...` — first two prologue bytes overwritten with `EB FE` = `jmp $`. The patch is **NOT from Utinni** (no detour RVA matches; `EB FE` is not a `DetourXS` encoding). Almost certainly SWGEmu's own runtime patcher neutralized this function — it's an **OS-version-detection routine** (likely `Os::install` / `SetupSharedFoundation`) that calls a kernel32/kernelbase API via IAT slot `[0x015DC0E0]` (runtime-resolved to `0x75381500` = kernel32 range, matches `GetVersion`/`GetVersionExA`), then decomposes the packed DWORD into major/minor/build at globals `0x019AA130-3C`, then validates via two `test eax,eax; jnz` branches with error codes `0x10` / `0x1C`. SWG's 2003 logic would bail on modern Win11 24H2 (`6.2.26100.x` reported as version 10.0+); SWGEmu patched the entry to halt instead of bail. SWGEmu presumably also patched out call sites; Utinni's injection re-triggers a code path that calls this function, halting the main thread. **Critical native-side finding** — investigate next: which Utinni hook or modified code path invokes the patched function. Possible approaches: (a) pattern-search SWGEmu.exe for `call <relative offset that resolves to 0x0131DC7A>` to find all (pre-patch) call sites, then verify with the source-built SWG client which call site is the one being newly reached; (b) ProcMon trace; (c) symbol-cross-reference via the other Claude with buildable SWG source — find the source function matching the GetVersion-decompose pattern, then find its callers. Vectored Exception Handler instrumentation already in place (commit `1ec9519`) but won't trigger since the halt isn't an exception — straight code-flow loop. **This is likely the root cause of #4 and #5** — if `Os::install` halts before character template load, the CLR exception in #4 is consequential (never actually reached in normal flow), and the SWG window in #5 is invisible because clientMain never returns from the halt to enter the render loop.
