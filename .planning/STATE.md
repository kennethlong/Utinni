...
last_activity: 2026-05-20 -- Issues #7+#8 confirmed RESOLVED via log scan (single-cycle hkSetScene/hkCleanupScene on TJT load path); NEW Issue #11 opened (in-game Return/Esc dead, WASD works)
progress:
  total_phases: 12
  completed_phases: 3
  total_plans: 9
...
11. **NEW 2026-05-20: In-game Return + Esc dead; WASD works** — Surfaced after Issue #9's login-screen fix verified end-to-end. User reaches the world (Tatooine), can move with WASD (DirectInput keyboard polling intact — log shows zero `DirectInput::suspend` events), but Return doesn't open the chat input box and Esc doesn't open the system menu. `/quit` therefore impossible. Clicking SWG window first doesn't help. Editor (FormMain) was visible/active during repro. **Key disconnect:** at the login screen post-Phase-B, Return worked (submitted credentials, log handoff confirms). In-game, Return is dead. So Issue #9 was real but incomplete — the in-game game-mode input path has a separate failure that the login-screen CUI path didn't expose. **Code static-analysis (2026-05-20):** `imgui_impl.cpp::hkWndProcHandler` forwards every WM_* unconditionally to SWG via `CallWindowProc`; `client.cpp` SWG WndProc detour is commented out; `Client::setHwnd` was never called (no diag log) so `Client::getHwnd()` stays null → `hkMainLoop` editor-mode resize branch auto-skips → SWG mainLoop gets its own HWND/dims. In code, WM_KEYDOWN should reach SWG. So either (a) messages reach the subclass and SWG itself isn't acting on them (DirectInput exclusive-mode keyboard, focus state, internal CUI gating), or (b) messages aren't reaching the subclass (intercepted earlier — global keyboard hook, exclusive-mode DirectInput consuming WM_KEYDOWN). **Next step:** diagnostic patch — tripwires in `hkWndProcHandler` for WM_KEYDOWN(VK_RETURN/VK_ESCAPE), WM_CHAR, WM_ACTIVATE/WM_SETFOCUS/WM_KILLFOCUS. User retests in-game with /quit attempt; log reveals (a) vs (b).
