...
last_activity: 2026-05-20 -- Issue #11 (in-game Return) RESOLVED via Phase H chatEnter handler override (commit 6047416); spun off Issue #12 (Esc system menu, separate dispatch chain) for follow-up
progress:
  total_phases: 12
  completed_phases: 3
  total_plans: 9
...
12. **NEW 2026-05-20: In-game Esc doesn't open system menu** — Surfaced during Issue #11 diagnostics. Esc DOES reach `CuiHud::actionPerformAction(0x00EDBAA0)` with `pThis=0x325DCC30` (CuiHud instance), `val1[7]=0x00000012` (action ID 18 = consistent every press), `vtbl=0x015F3964`, caller `0x00E2A46C`. Handler returns `ret=1` (success — action consumed) but no system menu visible. Per CODEX: `ret=1` means "action consumed", not "menu opened". Three hypotheses: (a) SystemMenu mediator activation failed / no-op'd at scene-init; (b) mediator activated but hidden/behind/not rendering; (c) action 0x12 isn't system menu in this state. **Next step:** instrument mediator activation. `UtinniCore/utinni.cpp:91` has `cuiMediatorFactorySetup::detour()` commented out — restoring or signature-scanning for activations by mediator name (`SwgCuiSystemMenu`, `GameMenu`, `gameMenuActivate`, etc.) is the disambiguation step. Lower priority since Phase H gives a working `/quit` path via chat — Esc dead is now a quality-of-life issue, not a blocker.
