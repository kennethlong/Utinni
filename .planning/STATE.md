...
last_activity: 2026-05-20 evening -- Issue #10 Phase B LANDED (commits 18e79c3 + 2ce028c). Borderless owned-popup reparenting works end-to-end -- SWG window pins inside PanelGame, frameless, owned by FormMain, drag-tracks. DirectInput coop-level baseline confirmed NONEXCLUSIVE|FOREGROUND and survives reparent unchanged. Phase H Enter + keyboard input intact. Position-only iteration (SWP_NOSIZE) -- D3D9 Reset for proper sizing deferred to Phase B-bis.
progress:
  total_phases: 12
  completed_phases: 3
  total_plans: 9
...
10. **SWG window reparenting — Phase A LANDED 2026-05-20 (commit `cbe1de7`); Phase B LANDED 2026-05-20 evening (commits `18e79c3` + `2ce028c`); Phase B-bis (proper sizing) PENDING.**
    ...
    - **Phase B verification 2026-05-20 evening (live SWG):** SWG window pinned at PanelGame top-left, frameless. DirectInput vtable shim shows the post-reparent `SetCooperativeLevel` call NEVER fires — owned-popup preserves top-level identity, DirectInput stays bound to the same HWND with identical flags. Phase H chatEnter override fires correctly (chat opens on Enter, /quit closes process cleanly). hkProcessEvent passes 28+ keyboard events for login dialog typing (Tab, letters, Enter). Dragging FormMain pulls SWG along — 8+ Reposition fires logged during drag with coords trending correctly. WASD specifically wasn't logged-confirmed (DirectInput polling has no instrumentation), but successful login + /quit strongly imply movement also works.
    - **Phase B-bis PENDING — D3D9 backbuffer reset for proper sizing:** current iteration uses `SWP_NOSIZE` — SWG stays at its native dimensions (probably 800x600 or whatever SWG.cfg specifies) pinned at PanelGame's top-left. First attempt that called `SetWindowPos` with `PanelGame.ClientSize` produced a black SWG window because D3D9's swapchain was created at SWG's native size and `Present()` doesn't auto-rescale on window resize in windowed mode (CODEX's "hkReset handles WM_SIZE naturally" prediction didn't pan out — that hook only fires on explicit `IDirect3DDevice9::Reset()`, and SWG doesn't call Reset() on window resize). **Approach for B-bis:** trigger `IDirect3DDevice9::Reset()` with `BackBufferWidth/Height` matching PanelGame's `ClientSize`, then let ImGui's invalidate/recreate cycle re-establish itself. Need to capture the device pointer (already done for D3D9 detours), build a fresh `D3DPRESENT_PARAMETERS` based on the current swapchain params with updated dimensions, call Reset(), handle `D3DERR_DEVICELOST`/`D3DERR_DEVICENOTRESET` round-trip. ImGui's `hkReset` at `directx9.cpp:323` already handles the invalidate side; we'd need to ensure `ImGui_ImplDX9_CreateDeviceObjects()` runs after Reset returns `D3D_OK`.
    - **References:** Microsoft `IDirect3DDevice9::Reset` docs, `D3DPRESENT_PARAMETERS` ref, ImGui dx9 backend's `ImGui_ImplDX9_InvalidateDeviceObjects` / `CreateDeviceObjects`.
