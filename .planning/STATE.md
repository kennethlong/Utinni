...
> ADVERTISED-CLIENT COVERAGE FOLLOW-ON — IN PROGRESS (2026-06-22). The follow-on milestone above is underway, split provider (swg-client-v2) / consumer (Utinni):
> - **Provider Phase 38 (delivered):** grew the GetEngineHookPoints contract 78→94 (UTINNI_HOOKPOINTS_VERSION 1→2) — groundScene×8, client/config×4, cuiChatWindow×4 — via __thiscall MI thunks + external-linkage shims. EPA-08 (DX11): static-analysis proof the gl11 provider populates swapChain+device+context together (D3D11CreateDevice@428 < CreateSwapChainForHwnd@574; destroy resets swapChain first) → the DX11 crash is consumer-side. Handback: `swg-client-v2/.planning/handoff/2026-06-22-utinni-provider-coverage-handback.md`.
> - **Consumer review finding (HIGH, blocks the gated wiring):** the provider advertised four DETOURED MI methods (groundScene::update, groundScene::handleInputMapEvent, cuiChatWindow::enableTextInput, cuiChatWindow::chatEnterHandler) as call-through thunks — a detour on a forwarder never intercepts the engine's direct calls (silent dead hook). Confirmed via Codex. Provider must re-advertise those four by their REAL entry (→ v3, names unchanged). Note + authoritative call-vs-detour table: `24-PROVIDER-FOLLOWUP-detour-vs-call.md` (copied to swg-client-v2 handoff). The called thunks + shims + config rows are detour-safe.
> - **Consumer work LANDED this session (Utinni master):** DX11 tryInstall hardening — poll until swapChain+device+context all non-null + one-shot 3-pointer diagnostic (`b43da2e`, the EPA-08 consumer fix). Virtual vtable-resolve primitive `swg/vtbl_resolve.h` + source-derived+Codex-validated indices (processEvent=1, addToWorld=2, removeFromWorld=3, setParentCell=13) + processEvent empirical SWGEmu self-check + headless `[vtbl]` unit test + HeaderDiscovery exclusion (`0a2fae9`). clang-format gate excludes the byte-identical shared contract header (`5d44b1f`). All native+ABI gates green; live DX11/advertised inject remains maintainer-only.
> - **Provider v3 DELIVERED + consumer post-v3 wiring LANDED (2026-06-22 PM):** provider Phase 38-05 re-advertised the 4 detoured endpoints by REAL engine entry (pmfRealEntry/real-entry accessors, delta==0 verified) → contract v3 / 94 names (swg-client-v2 pushed). Consumer re-synced v3 + bound the 16 new endpoints (`d3b5468`), bumped asserts 78→94. Two intentional CARVE-OUTS: consoleHelper::sendInput (D-02) + **client::wndProc** (`4072eb1` — binding it forwarded the TJT panel's WM_SIZE to the embedded DX11 client → wrong ResizeBuffers → corrupted render). groundScene+cuiChatWindow got per-target installable() gating; a SELECTIVE advertised-client unlock of those two was tried + REVERTED (`ff7e80e`) — live-smoke showed per-HOOK runtime crashes (setPreloadSnapshot data-global write `fe97a52`, then a client std::string fault) that installable()/memory-guards can't catch.
> - **DX11 advertised-client live arc (2026-06-22 PM):** overlay install FIXED (ImGui context ordering `7ed9403`); ASLR-roulette startup crash FIXED (getSwgWndProcExport returns null on the advertised client `c842b84`). **Current state: advertised DX11 client BOOTS reliably + overlay installs + content renders correctly, but the embed is MIS-SIZED** (login black bar / in-game redraw tearing). Backbuffer 1600×900, swapchain already DXGI_SCALING_STRETCH, client::wndProc carved out → embedded window/RTV/viewport don't match the TJT panel on the DX11 path (D3D9 RT-space stretch masked it).
> - **TWO RNDR-04 / per-hook FOLLOW-ONS (both maintainer-live, NOT headless):** (1) **DX11 embed-resize** — make the reparented window + RTV/viewport fill the panel; instrumented RenderDoc debug session, plan in `24-DX11-EMBED-RESIZE-DEBUG-PLAN.md`. (2) **Editor unlock on the advertised client** — per-hook ABI verification before dropping the MISC/INPUT group-skip (a wholesale drop is unsafe: game::mainLoop→Game::run ABI mismatch crashes; CuiManager render+findObjectUnderCursor needs per-target gating). Bindings + per-target gating infrastructure are in place for both.
> - **Repo ownership:** the Utinni instance owns all `D:/Code/Utinni` consumer edits; the provider instance stands down on the Utinni repo (maintainer-confirmed 2026-06-22).
> - **Hard-won guardrail (this session):** anything touching the advertised client's window/detour/render behavior needs a maintainer LIVE smoke before commit — installable()/headless builds + ABI gates cannot catch the ABI / ASLR / embed-state / render-correctness failure modes. Several speculative advertised-client commits regressed boots before reverting. Durable wins (v3 wiring, resolver coverage, overlay install, the crash fixes) are all committed + CI-green; the remaining work is the two live-tuned follow-ons above.
